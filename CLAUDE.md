# MSP AI — Project Constitution

> Private AI operations platform for **IT MSPs**.
...
A **secure, read-only conversational assistant** the the MSP team chats with (text + voice) to check things
...
**We win v1 when:** a MSP tech can open the dashboard, pick a client (or "all"), ask a plain-English
...
```jsonc
{ "ts": "ISO-8601", "actor": "user@example", "tenant_id": "acme",
...
6. **No free-form shell for the AGENT, ever.** The agent runs only registered, parameterized tools.
   ...
   **admin-only, audited** Terminal lets a logged-in admin run commands on the host (a convenience over
   ...
   runs, and `MSPAI_ADMIN_TERMINAL=0` kills it instantly (I-4). The agent's no-shell rule is unchanged;
...
- **Native agent loop (the brain).** `execution/agent.py` is MSP AI's own bounded tool-call loop —
  ...
  (`default`) + specialists under `profiles/<name>/`, resolved from `MSPAI_AGENTS_DIR` (legacy
  `MSPAI_HERMES_*` fallback until migrated) else `<vault>/agents`. **Delegation** = the native
