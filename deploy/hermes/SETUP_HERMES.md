# Hermes Agent ⇄ MSP AI — Setup Guide (Ubuntu)

Wire **Nous Research Hermes Agent** to MSP AI as a *fenced brain*: Hermes gets memory,
self-improving skills, and multi-channel reach, but it can only touch client systems
through MSP AI's guarded tools (read-only by default, capability/approval/tenant/audit all
enforced by MSP AI's `dispatch()` — see `../../architecture/hermes-integration.md`).

> Run these on the Ubuntu server where the local LLM lives. Assumes MSP AI is at
> `/opt/msp-ai` (set `MSPAI_AI_HOME` otherwise). Sources at the bottom.

---

## 0. Prerequisites
- MSP AI deployed at `/opt/msp-ai` and working: `cd /opt/msp-ai && python3 -m execution.cli probe`
...
MSP AI's own router already enforces local-first for client data independently.)

## 3. Make the MSP AI launcher executable
```bash
chmod +x /opt/msp-ai/deploy/hermes/msp-ai-mcp.sh
# sanity check (Ctrl-D after it prints nothing / waits):
echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | /opt/msp-ai/deploy/hermes/msp-ai-mcp.sh --tenant acme
#  → should print a JSON line listing the MSP AI tools
```

## 4. Register MSP AI as an MCP server
Edit `~/.hermes/config.yaml` and paste the `mcp_servers` block from
[`config.snippet.yaml`](config.snippet.yaml) (one `mspai_<client>` entry per client). Then,
inside a Hermes session:
```
/reload-mcp
```
Hermes connects and exposes the tools as `mcp_mspai_acme_kaseya_list_assets`, etc.
...
```
Keep the MSP AI MCP tools enabled; that's how Hermes does MSP work — safely.

## 6. Verify end-to-end
In a Hermes session (msp profile):
```
> Using the MSP AI tools, how many assets does the acme client have, and any open Huntress incidents?
```
Confirm Hermes calls `mcp_mspai_acme_kaseya_list_assets` / `mcp_mspai_acme_huntress_list_incidents`,
then check the MSP AI audit log: `cd /opt/msp-ai && python3 -m execution.cli audit --tenant acme`.
...
toolset minimal; widen capabilities only as trust grows — in BOTH places: MSP AI's Capability
Console (the MSP tools) and `hermes tools` (Hermes' native tools).

---
## The two control planes (where you turn things on)
- **MSP AI Capability Console** (dashboard) → governs the *MSP tools* that touch client systems
