...
MSP AI MCP tools (which are guarded). Hermes' own native tools that act locally/on the host start
OFF and open only as you validate.**

This is *guidance for `hermes tools` / `~/.hermes/config.yaml`* — Hermes controls its own native
toolsets. MSP AI's Capability Console controls the MSP tools (the ones that reach client systems).

| Hermes toolset | What it does | MSP start | Notes |
|---|---|---|---|
| **MSP AI MCP tools** (`mcp_mspai_*`) | Guarded MSP reads + memory | **ON** | The safe path; every call hits MSP AI dispatch(). |
| `memory` / `session_search` | Hermes' own cross-session memory | ON | Complements MSP AI's per-client `memory.md`. |
...
1. **Reads only** — MSP AI MCP read tools + Hermes `memory`/`web`. Prove answers are correct + sourced.
2. **Internal writes** — MSP AI `memory_note` (already safe). Let Hermes build per-client memory.
3. **Guarded MSP writes** — when MSP AI's approval workflow ships, open a specific MSP AI write tool
