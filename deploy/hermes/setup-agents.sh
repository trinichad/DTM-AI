#!/usr/bin/env bash
# Create the DTM AI specialist agent team as Hermes profiles, each with its SOUL + a
...
#
# Run on the server (as ross — needs docker exec + sudo -u dtm-ai):
#   bash /opt/dtm-ai/deploy/hermes/setup-agents.sh
set -euo pipefail
SOULS=/opt/dtm-ai/deploy/hermes/souls
...
for name in "${names[@]}"; do
  ...
  sudo -n -u dtm-ai cp "$SOULS/$name.md" "$DATA/profiles/$name/SOUL.md"
done

# AtlasOps Manager = the default profile (the active one chat/api_server serves)
sudo -n -u dtm-ai cp "$SOULS/atlasops.md" "$DATA/SOUL.md"
