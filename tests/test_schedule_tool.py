...
class ScheduleTool(unittest.TestCase):
    def setUp(self):
        ...
        self._prev = os.environ.get("DTM_AGENTS_DIR")
        os.environ["DTM_AGENTS_DIR"] = str(d)
        self.store = TaskStore(d / "t.db")
        self.ctx = ToolContext(tenant_id="acme", actor="chad@dtm", _meta={"tasks": self.store})

    def tearDown(self):
        self.store.close()
        if self._prev is None:
            os.environ.pop("DTM_AGENTS_DIR", None)
        else:
            os.environ["DTM_AGENTS_DIR"] = self._prev
        self.tmp.cleanup()

    def test_creates_recurring_scheduled_task(self):
        r = schedule_task.run(self.ctx, title="RHO Cylance check", assignee="patchwright",
