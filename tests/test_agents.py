...
class Agents(unittest.TestCase):
    def setUp(self):
        ...
        write(pp / "USER.md", "the MSP MSP team.\n")
        write(pp / "skills" / "cat" / "s" / "SKILL.md", "x")
        self.cfg = StubCfg({"MSPAI_AGENTS_DIR": str(self.d)})   # native key
    ...
    def test_data_dir_resolution(self):
        # native key wins
        self.assertEqual(_data_dir(StubCfg({"MSPAI_AGENTS_DIR": "/x/agents"})), Path("/x/agents"))
        # legacy Hermes data dir still honored (deploy keeps working until migrated)
        self.assertEqual(_data_dir(StubCfg({"MSPAI_HERMES_DATA_DIR": "/srv/hermes-data"})),
                         Path("/srv/hermes-data"))
        # legacy skills dir → its parent
        self.assertEqual(_data_dir(StubCfg({"MSPAI_HERMES_SKILLS_DIR": "/srv/hermes-data/skills"})),
                         Path("/srv/hermes-data"))
        # nothing set → <vault>/agents
        self.assertEqual(_data_dir(StubCfg({"MSPAI_VAULT_PATH": "/v"})), Path("/v/agents"))
...
class CrewStudio(Agents):
    ...
    def test_agent_memory_note_tool_uses_running_profile(self):
        ...
        prev = os.environ.get("MSPAI_AGENTS_DIR")
        os.environ["MSPAI_AGENTS_DIR"] = str(self.d)
        try:
            ...
            if prev is None: os.environ.pop("MSPAI_AGENTS_DIR", None)
            else: os.environ["MSPAI_AGENTS_DIR"] = prev
