...
class WebApi(unittest.TestCase):
    ...
    def test_codex_oauth_signin_flow(self):
        """'Sign in with ChatGPT' device flow (D-26) — start, pending poll, connect."""
        from unittest.mock import patch
        self.assertEqual(self.H("POST", "/api/integrations/openai_codex/oauth/start").status, 401)
        with patch("execution.core.codex_auth.start_device_auth",
                   return_value={"device_auth_id": "da1", "user_code": "AB-12",
                                 "verification_url": "https://auth.openai.com/codex/device",
                                 "interval": 5, "expires_at": "x"}):
            r = self.H("POST", "/api/integrations/openai_codex/oauth/start", user="admin")
        self.assertEqual(r.status, 200)
        self.assertEqual(r.payload["user_code"], "AB-12")
        self.assertEqual(self.H("POST", "/api/integrations/openai_codex/oauth/poll",
                                {}, user="admin").status, 400)
        with patch("execution.core.codex_auth.poll_device_auth", return_value=("pending", None)):
            r = self.H("POST", "/api/integrations/openai_codex/oauth/poll",
                       {"device_auth_id": "da1", "user_code": "AB-12"}, user="admin")
        self.assertEqual(r.payload["status"], "pending")
        with patch("execution.core.codex_auth.poll_device_auth",
                   return_value=("ok", {"authorization_code": "ac", "code_verifier": "cv"})), \
             patch("execution.core.codex_auth.exchange_device_code", return_value=("tok", "acct")):
            r = self.H("POST", "/api/integrations/openai_codex/oauth/poll",
                       {"device_auth_id": "da1", "user_code": "AB-12"}, user="admin")
        self.assertEqual(r.payload["status"], "connected")
        self.assertIn("OPENAI_CODEX_ACCESS_TOKEN", r.payload["fingerprints"])
    ...
    def test_tool_add_move_delete_roundtrip(self):
        # D-49: capability delete/move work (regression for the file-ownership breakage). add a
        # throwaway tool, move its group, delete it.
        code = ('NAME = "zz_test_tool"\nDESCRIPTION = "x"\nSOURCE = "misc"\nCATEGORY = "read"\n'
                'PARAMETERS = {"type": "object"}\ndef run(ctx, **k):\n    return {}\n')
        try:
            self.assertTrue(self.H("POST", "/api/tools",
                                   {"name": "zz_test_tool", "code": code}, user="admin").payload["ok"])
            mv = self.H("POST", "/api/tools/zz_test_tool/source", {"source": "newgrp"}, user="admin")
            self.assertEqual(mv.payload.get("source"), "newgrp")
            dl = self.H("DELETE", "/api/tools/zz_test_tool", user="admin")
            self.assertTrue(dl.payload["ok"])
            self.assertIsNone(self.agent.registry.get("zz_test_tool"))   # gone from the registry
        finally:
            import os
            for p in ("execution/skills/zz_test_tool.py", ".tmp/deleted_skills/zz_test_tool.py"):
                try: os.unlink(p)
                except OSError: pass

    def test_fs_error_message_guides_chown(self):
        # A blocked skill-file op returns an actionable chown hint, not a bare errno.
        msg = Api._fs_error("delete", type("P", (), {"name": "x.py"})(),
                            PermissionError(13, "Permission denied"))
        self.assertIn("chown", msg)
        self.assertIn("msp-ai", msg)

    def test_detect_single_client_for_autolock(self):
        # D-52: deterministic, model-independent lock — exactly one named client → that client.
        clients = ["Acme_Test", "Acme_Corp", "Unnamed_Group"]
        d = Api._detect_single_client
        self.assertEqual(d("list users in acme_test tenant", clients), "Acme_Test")
        self.assertEqual(d("Acme Corp devices?", clients), "Acme_Corp")
        self.assertEqual(d("acme-test backups", clients), "Acme_Test")        # separators flexible
        self.assertIsNone(d("how many devices across all clients?", clients))  # none named
        self.assertIsNone(d("compare Acme_Test and Acme_Corp", clients)) # two → ambiguous
        self.assertIsNone(d("", clients))

    def test_chat_stop_is_owner_scoped(self):
        # D-45: a user can only stop a conversation they own; unknown/foreign → 404.
        cid = self.H("POST", "/api/chat", {"tenant": "acme", "message": "hi"},
                     user="admin").payload["conversation_id"]
        self.auth.create_user("bob", "bobpass1", "user")
        self.assertEqual(self.H("POST", "/api/chat/stop", {"conversation_id": cid},
                                user="bob").status, 404)               # not bob's conversation
        self.assertEqual(self.H("POST", "/api/chat/stop", {"conversation_id": "nope"},
                                user="admin").status, 404)             # unknown id
        r = self.H("POST", "/api/chat/stop", {"conversation_id": cid}, user="admin")
        self.assertEqual(r.status, 200)
        self.assertTrue(r.payload["ok"])
        self.assertFalse(r.payload["stopping"])                        # no turn currently in flight
    ...
    def test_service_restart_admin_only_and_detached(self):
        from unittest import mock
        self.auth.create_user("bob", "bobpass1", "user")
        self.assertEqual(self.H("POST", "/api/system/restart").status, 401)        # unauth
        self.assertEqual(self.H("POST", "/api/system/restart", user="bob").status, 403)  # non-admin
        with mock.patch("subprocess.Popen") as popen:
            r = self.H("POST", "/api/system/restart", user="admin")
        self.assertEqual(r.status, 200, r.payload)
        self.assertTrue(r.payload["restarting"])
        popen.assert_called_once()
        argv = popen.call_args[0][0]
        self.assertIn("systemctl", argv)
        self.assertIn("restart", argv)
        self.assertTrue(popen.call_args.kwargs.get("start_new_session"))   # survives our death
...
class Branding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.auth.ensure_admin("secret")
        self.api = Api(self.agent, self.auth, SessionSigner(secret=b"0" * 32))
        self._prev = os.environ.get("MSPAI_BRANDING_DIR")
    ...
    def tearDown(self):
        ...
        if self._prev is None:
            ...
            os.environ["MSPAI_BRANDING_DIR"] = self._prev
        self.auth.close()
        self.tmp.cleanup()
...
class CredVaultApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        db = Path(self.tmp.name) / "w.db"
        self._prev = os.environ.get("MSPAI_VAULT_PATH"); os.environ["MSPAI_VAULT_PATH"] = self.tmp.name
        self.agent = build_agent(db_path=db)
    ...
    def tearDown(self):
        import os
        if self._prev is None: os.environ.pop("MSPAI_VAULT_PATH", None)
        else: os.environ["MSPAI_VAULT_PATH"] = self._prev
