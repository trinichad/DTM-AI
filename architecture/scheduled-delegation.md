...
Each profile may pin a **brain** = a router model id (`ollama:qwen3.5:27b`,
`anthropic:claude-opus-4-8`). Stored native in a one-line sidecar `(<profile dir>)/brain`, NOT in
...
`SOURCE="msp_ai"`. It reaches the board via `ctx._meta["tasks"]` (the TaskStore injected at
