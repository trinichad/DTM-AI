...
## Goal
Give MSP AI a knowledge base + per-client long-term memory so it behaves like an employee who
...
## Layout
```
vault/                          # path = MSPAI_VAULT_PATH (default <project>/vault); GITIGNORED (client data)
...
- **Internal write ≠ client-system write.** `memory_note`/`memory_update` write ONLY to our vault, so they
  ...
  floor governs CLIENT systems, not MSP AI's own memory.
...
- **`vault/kb/`** — the owner's own runbooks/SOPs (under `MSPAI_VAULT_PATH`, gitignored, per-deployment,
...
- **`reference/`** (repo root, **git-tracked**) — bundled vendor references that ship WITH the app and
  ...
  from help.vsa9.kaseya.com — REFERENCE ONLY, not executable; MSP AI v1 is read-only).
...
## Amendment (2026-06-10, D-31) — per-USER profile memory

`vault/users/<username>.md` is what the agent knows about each HUMAN (preferred email, phone,
preferences). Injected into the system prompt every turn ("The person you are talking to") along
with the account email, so "email me" resolves correctly — `send_email` also accepts `to="me"`.

Write path: `user_memory_note` (append a fact) / `user_memory_update` (confirmed rewrite; .bak
kept). Both are bound to the SIGNED-IN user from ctx — the model cannot target another person's
profile. Conflict protocol (in the prompt): a new statement that contradicts a stored fact (e.g. a
different email) must NOT be silently overwritten — the agent shows the stored value and asks
UPDATE / KEEP / ADD, then applies the choice. Owners edit their own profile in Settings → "What
the agent knows about me" (`GET/POST /api/me/memory`).

Teams identity: an allowlist entry may link a dashboard account (`aad-id|Name|username`, set in
the Teams card UI) — Teams turns then run with THAT user's email + profile. Unlinked Teams users
get a `teams:<aad-id>`-scoped profile with no email.
