...
_STOP = {"the", "a", "an", "of", "to", "for", "and", "or", "in", "on", "with", "by", "this",
         "that", "is", "are", "use", "using", "when", "client", "clients", "dtm", "all"}
...
class PlaybookStore:
    ...
    def __init__(self, path: Optional[Path] = None, cfg: Optional[Config] = None) -> None:
        ...
        vault = Path(path) if path else Path(cfg.get("DTM_VAULT_PATH") or (_PROJECT_ROOT / "vault"))
    ...
    def save(self, name: str, description: str = "", tools=None, when: str = "",
             steps: str = "", tags=None, created_by: str = "dtm-ai", force: bool = False) -> dict:
        name = (name or "").strip()
        if not name:
