...
A delegation task is assigned to a specialist profile and executed by a worker that runs MSP AI's
OWN agent loop AS that profile (its SOUL + memory + the same guarded tools) — no external runtime,
no `docker exec`, no privileged sudo wrapper. The board is a normal MSP AI store (SQLite dev →
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    created_by    TEXT DEFAULT 'msp-ai',
    tenant        TEXT DEFAULT '',
...
class TaskStore:
    ...
    # ── writes ──
    def create(self, title: str, body: str = "", assignee: str = "", created_by: str = "msp-ai",
        ...
        with self._lock:
            ...
            self._conn.execute(
                ...
                (tid, title, body or "", assignee, status, created_by or "msp-ai",
...
class Dispatcher:
    ...
    def __init__(self, store: TaskStore, agent: Any,
        ...
        # model_resolver(profile) -> router model id or None. Lets a specialist run on its own pinned
