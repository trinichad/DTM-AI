...
Ported from the the donor design design (its execution/terminal.py), adapted from asyncio to threads + select
so it fits MSP AI's stdlib ThreadingHTTPServer (one thread per connection). Forks a real login shell on a
pseudo-terminal so the browser can host full TUIs — vim, top, interactive `claude`, password prompts,
colors. Runs as whatever user the server process is (msp-ai on :8090; root on :8091). Stdlib only.

Wire protocol (JSON text frames, matches the donor):
...
def serve(sock, cwd: str = None) -> None:
    ...
    if pid == 0:  # child → become the login shell
        ...
        os.environ["MSPAI_TERMINAL"] = "1"
        if os.geteuid() == 0:
            # root has no ~/.bashrc, so /etc/bash.bashrc leaves a monochrome PS1. Install the same
            # Debian colored prompt the msp-ai user gets on :8090; self-removes before the first prompt.
            os.environ["PROMPT_COMMAND"] = (
                r"PS1='${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:"
                r"\[\033[01;34m\]\w\[\033[00m\]\$ '; unset PROMPT_COMMAND"
            )
        try:
            os.chdir(cwd)
        except OSError:
