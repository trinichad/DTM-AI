...
def build_agent(cfg: Optional[Config] = None, db_path: Optional[Path] = None) -> Agent:
    ...
    # Seed internal-write policy: MSP AI's OWN tools (source=msp_ai) that write only touch our
    ...
    from .core.credvault import get_credvault
    agent.credvault = get_credvault(cfg)         # encrypted per-client credential vault (D-25);
    # the SHARED instance (D-37) — m365_auth stores its tokens through the same in-memory DEK
