...
def run(ctx, identity: str, **_: Any):
    ...
    if bool(mb.get("AutoExpandingArchiveEnabled")):
        return {"ok": True, "mailbox": primary,
    ...
    if not guid or guid == _NO_ARCHIVE_GUID or str(mb.get("ArchiveState")) == "None":
        return {"ok": False, "error": f"'{primary}' has no online archive yet — enable it "
    ...
    if c.err(r):
        ...
        return {"ok": False, "step": "enable", "error": e + hint}
    ...
    return {"ok": True, "mailbox": primary, "auto_expanding_archive": "enabled",
