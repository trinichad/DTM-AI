...
DESCRIPTION = ("Update a Freshdesk ticket by `ticket_id`. Change any of: status (open/pending/"
               ...
               "freshdesk_reply_ticket; to add an internal note use freshdesk_add_note.")
SOURCE = "freshdesk"
...
def run(ctx, ticket_id: int, status: str = "", priority: str = "", type: str = "",
        group_id: Any = None, agent_id: Any = None, tags: Any = None, **_: Any):
    tid = int(ticket_id)
    body: dict[str, Any] = {}
    if (status or "").strip():
