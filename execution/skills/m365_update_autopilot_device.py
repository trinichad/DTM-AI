...
DESCRIPTION = ("Update an EXISTING Autopilot device: set/change its group tag, assigned user, "
               "or display name. Find the device by serial with m365_list_autopilot_devices. "
               "Intune applies the change asynchronously — re-check the list afterwards.")
SOURCE = "m365"
CATEGORY = "write"
RISK_LEVEL = "medium"
...
def run(ctx, serial: str, group_tag: str = "", assigned_user: str = "",
    ...
    if not body:
        return {"ok": False, "error": "give group_tag, assigned_user, and/or display_name"}
    ...
    try:
        ...
        if bad:
            return bad
        if not dev:
            return {"ok": False, "error": f"no Autopilot device with serial '{serial}' — "
